...
impl LlmConfig {
    ...
    pub fn from_env(get: impl Fn(&str) -> Option<String>) -> Self {
        ...
        LlmConfig {
            url: non_empty("MOXIN_LLM_URL").unwrap_or_else(|| DEFAULT_URL.to_string()),
            model: non_empty("MOXIN_LLM_MODEL").unwrap_or_else(|| DEFAULT_MODEL.to_string()),
            api_key: non_empty("MOXIN_LLM_API_KEY"),
            key_header: non_empty("MOXIN_LLM_KEY_HEADER")
...
/// 构造 Anthropic Messages API 请求体。OpenAI 兼容端点结构不同,M1 先只支持
/// Messages 形态(靠 `MOXIN_LLM_KEY_HEADER` 切 `Authorization` 已能打通多数网关)。
...
/// 从 Anthropic 响应抽出文本。错误响应(`{"error":{"message":...}}`)→ `Err`。
pub fn parse_answer(resp: &str) -> Result<String> {
    ...
    let content = v
        ...
        .ok_or_else(|| anyhow!("LLM 响应缺少 content 数组"))?;
...
fn render_curl_config(cfg: &LlmConfig, api_key: &str) -> String {
    let lines = [
        ...
        format!(
            "header = \"{}: {}\"",
            curl_escape(&cfg.key_header),
            curl_escape(api_key)
        ),
        format!("max-time = {}", CURL_TIMEOUT_SECS),
        "silent".to_string(),
...
mod tests {
    ...
    #[test]
    fn parse_answer_extracts_text_blocks() {
        let resp = r#"{"content":[{"type":"text","text":"D13 is toggling"},{"type":"text","text":" at 1Hz."}]}"#;
        assert_eq!(parse_answer(resp).unwrap(), "D13 is toggling at 1Hz.");
    }

    #[test]
    fn parse_answer_surfaces_api_error() {
        let resp = r#"{"error":{"type":"authentication_error","message":"invalid x-api-key"}}"#;
        let e = parse_answer(resp).unwrap_err().to_string();
        assert!(e.contains("invalid x-api-key"), "got: {e}");
    }

    #[test]
    fn parse_answer_rejects_garbage_and_empty() {
        assert!(parse_answer("not json").is_err());
        assert!(parse_answer(r#"{"content":[]}"#).is_err());
        assert!(parse_answer(r#"{"content":[{"type":"text","text":"  "}]}"#).is_err());
    }

    #[test]
    fn curl_config_escapes_and_carries_key_header() {
        let cfg = LlmConfig::from_env(|k| match k {
        ...
        assert!(rendered.contains(DEFAULT_URL));
    }
